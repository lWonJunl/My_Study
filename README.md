...
| Java | 출력, 변수와 자료형, 연산, 조건문, 반복문, 누적 합계 |
| Frontend | HTML/CSS 기초, JavaScript, DOM 조작, 이벤트 처리 |
| Database | SQL 데이터 조회, 조건 검색, 정렬 |
...
| `08_study_time_counter/` | 이벤트 리스너를 활용한 공부 시간 카운터 |
...
| 파일 | 학습 주제 |
| --- | --- |
| `SQL01SelectWhere.sql` | `SELECT`와 `WHERE`로 데이터를 조회하고 `ORDER BY`로 오름차순·내림차순 정렬하기 |

## 학습 목표

- C, Python, Java의 기본 문법 익히기
- 알고리즘 문제 풀이를 통해 문제 해결 능력 기르기
- HTML, CSS, JavaScript의 역할과 상호작용 이해하기
- DOM 조작과 이벤트 처리로 동적인 웹 페이지 구현하기
- SQL을 활용해 필요한 데이터를 조회하고 조건에 맞게 검색·정렬하기
